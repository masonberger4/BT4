...
| RiboNN round-2 pre-registration panel | `scripts/data/prereg_round2_panel.json` | 16 human proteins + 4 UTR contexts drawn by seeded rule from **NCBI MANE release 1.5** (`MANE.GRCh38.v1.5.summary.txt.gz`, SHA-256 pinned in the file), with protein / 5′UTR / 3′UTR sequences fetched from **Ensembl** REST for the named MANE Select transcripts; built by `scripts/prereg_round2_draw_panel.py` (`--verify` re-draws and diffs) | **MANE** is a joint NCBI/EMBL-EBI project; the summary table is a US Government work in the **public domain** ([NCBI policies](https://www.ncbi.nlm.nih.gov/home/about/policies/)) — cite Morales et al., *Nature* 2022, doi:10.1038/s41586-022-04558-8. Sequences are **Ensembl** (same terms and citation as the codon-table rows above). No measured data and no model output — reference sequence only. |
