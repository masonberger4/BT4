...
> `ConstructContext` changes the answer rather than refining it. *(Scoped 2026-08-22: it is
> a **lower** bound against real genomic flank only. Against designed-CDS flank the same
> planted donor falls 0.6554 → 0.3742 across 1 kb, so the padded number over-reads there.)*
> It licenses **no** threshold change — there are no labels, and a higher score is not a more correct one.
>
