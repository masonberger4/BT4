...
def _isolate_process_wide_switches() -> Iterator[None]:
    ...
    enabled. On CI that was invisible -- no CNN weights are installed, so
    ``promote_if_attested`` is a no-op and the assertions still passed. On a machine
    holding the licensed weights it silently promoted Pangolin to
    ``calibrated=True`` across the suite, which is precisely the "promotion leaked
    into the default path" failure the opt-in design exists to prevent, and it turned
    the ``all_calibrated is False`` assertions into environment-dependent tests.
