#!/usr/bin/env python
"""Draw the frozen protein / UTR panel for RiboNN Part 3 round 2.
...
(`docs/PREREG_ribonn_part3_round2.md`) is written to prevent, so the draw is
...
Usage::
    ...
    python scripts/prereg_round2_draw_panel.py --out scripts/data/prereg_round2_panel.json
    python scripts/prereg_round2_draw_panel.py --verify scripts/data/prereg_round2_panel.json
"""
...
def _curl(url: str) -> bytes:
    ...
    out = subprocess.run(  # noqa: S603
        ["curl", "-sSL", "--fail", "--max-time", "300", url],  # noqa: S607
        capture_output=True,
        check=True,
    )
    return out.stdout
    ...
    for attempt in range(tries):
        try:
            ...
            if exc.code in (400, 404):  # a definitive "no such thing"
                raise
            last = exc
        except Exception as exc:  # noqa: BLE001 - transient network or local SSL
            last = exc
            try:
                return _curl(url)
...
def fetch_transcript(enst: str) -> dict[str, Any] | None:
    ...
    base = enst.split(".")[0]
    try:
        cdna = _fasta_body(_get(f"{ENSEMBL}/sequence/id/{base}?type=cdna;content-type=text/x-fasta"))
        cds = _fasta_body(_get(f"{ENSEMBL}/sequence/id/{base}?type=cds;content-type=text/x-fasta"))
    except Exception:  # noqa: BLE001 - a missing or odd transcript is a skip, not a crash
        return None
...
def main() -> int:
    ...
    print(
        "UTR contexts: "
        + ", ".join(f"{c['name']}(5'={c['utr5_len']},3'={c['utr3_len']})" for c in manifest["utr_contexts"])
    )
    return 0
